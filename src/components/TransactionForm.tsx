...
import { Plus, Camera, Loader2, CheckCircle, AlertCircle, X, Sparkles } from 'lucide-react';
...
async function parseBillImage(base64Image: string, mimeType: string): Promise<ParsedBill> {
  ...
  const prompt = `
Extract transaction details from this bill.
...
interface UploadPanelProps {
  onParsed: (result: ParsedBill) => void;
}
...
const resizeImage = (file: File): Promise<string> => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      const scale = Math.min(1, maxWidth / img.width); // prevent upscaling
...
function ImageUploadPanel({ onParsed }: UploadPanelProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    setError(null);
    setParsed(null);
    setMimeType('image/jpeg'); // we convert everything to jpeg
    ...
    try {
      ...
      const result = await parseBillImage(base64, 'image/jpeg');
      ...
      console.error(err);
      setError('Failed to parse image. Please fill in manually.');
    } finally {
      setParsing(false);
    }
  };
    ...
    <div className="space-y-3">
      ...
      {!preview && (
        ...
        >
          ...
          <p className="text-sm font-medium text-foreground">Upload Bill / Receipt</p>
          <p className="text-xs text-muted-foreground mt-1">Tap to capture or drag & drop</p>
          <p className="text-xs text-muted-foreground">Works with photos, screenshots, handwritten bills</p>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <img
              src={preview}
              alt="Bill preview"
              className="w-full max-h-48 object-contain rounded-xl border border-border/40 bg-muted/20"
            />
            ...
            <div className="bg-success/8 border border-success/20 rounded-lg px-3 py-2.5 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5 mb-1">
                ...
                {parsed.confidence && (
                  <Badge
                    variant="outline"
                    className={cn('text-[10px] ml-auto', confidenceColor[parsed.confidence])}
                  >
              ...
              <p className="text-[10px] text-muted-foreground pt-1">
                ↓ Fields pre-filled below — review & edit before saving
              </p>
            </div>
          )}
...
export default function TransactionForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  // When AI parses a bill, pre-fill relevant form fields
  const handleParsedBill = (result: ParsedBill) => {
    setForm(f => ({
      ...
      // Only set category if it exists in our list
      ...
      <DialogContent className="max-w-md mx-auto max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-1 bg-muted/50 rounded-lg p-1">
          ...
          >
            ...
            Scan Bill
            <span className="text-[9px] bg-primary/15 text-primary px-1 py-0.5 rounded-full font-semibold">AI</span>
          </button>
        </div>

        <div className="space-y-3">

          {/* ── IMAGE UPLOAD PANEL (shown only on image tab) ── */}
          {tab === 'image' && (
            <ImageUploadPanel onParsed={handleParsedBill} />
          )}

          {/* Divider when in image tab */}
          {tab === 'image' && (
          ...
          {/* ── TRANSACTION FIELDS (always shown) ── */}
