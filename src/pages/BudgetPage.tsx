...
export default function BudgetPage() {
  ...
  return (
    <div className="pb-20 px-2 sm:px-4 pt-4 max-w-2xl mx-auto space-y-4 animate-slide-up">
      ...
      {/* ── TOP 3 SUMMARY CARDS: Expected / Actual / Balance (combined Home+Debt) ── */}
        ...
        <div className="min-w-[520px]">
          ...
          <div className="space-y-1">
            {allHomeCategories.map(c => {
              ...
              return (
                <div key={c.category} className={cn('rounded-lg px-2 py-2 transition-colors',
              ...
              return (
                <div key={cat} className={cn('rounded-lg px-2 py-2', over ? 'bg-destructive/8' : '')}>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={cn('w-24 text-right text-xs font-bold shrink-0',
                      bal >= 0 ? 'text-success' : 'text-destructive')}>
                      {fmtBal(bal)}
                    </span>
                    <span className={cn('w-20 text-right text-xs font-bold shrink-0',
                      overspendAmt > 0 ? 'text-destructive' : 'text-muted-foreground')}>
      ...
      <div className="glass-card rounded-xl p-4 mx-2 border border-destructive/20 space-y-4">
        ...
        <div className="rounded-xl bg-muted/30 p-3 space-y-1.5 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold tabular-nums">{fmt(homeExpected)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Total Home Actual</span>
            <span className="font-semibold text-destructive tabular-nums">{fmt(homeActual)}</span>
          </div>
          <div className="flex justify-between border-t border-border/40 pt-1.5 font-bold text-sm">
            <span>Balance in Home</span>
            <span className={cn('tabular-nums', homeBalance >= 0 ? 'text-success' : 'text-destructive')}>
              {fmtBal(homeBalance)}
            </span>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-muted/30 p-3 space-y-1.5 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold tabular-nums">{fmt(debtExpected)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">Total Debt Actual</span>
            <span className="font-semibold text-destructive tabular-nums">{fmt(debtActual)}</span>
          </div>
          <div className="flex justify-between border-t border-border/40 pt-1.5 font-bold text-sm">
            <span>Balance in Debt</span>
            <span className={cn('tabular-nums', debtBalance >= 0 ? 'text-success' : 'text-destructive')}>
              {fmtBal(debtBalance)}
            </span>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-primary/8 border border-primary/20 p-3 space-y-1.5 text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-destructive tabular-nums">{fmt(overallActual)}</span>
          </div>
          <div className="flex justify-between border-t border-border/40 pt-1.5 font-bold text-sm">
            <span>Balance</span>
            <span className={cn('tabular-nums', overallBalance >= 0 ? 'text-success' : 'text-destructive')}>
              {overallBalance >= 0
                ? `Under by ${fmt(overallBalance)}`
                : `Over by ${fmt(Math.abs(overallBalance))}`}
            </span>
          </div>
          {homeOverspend > 0 && (
