...
export default function PersonWisePage() {
  ...
  // ── Internal Transfer form (Feature 15) ───────────────────────────────────
  ...
  const doTransfer = async () => {
    ...
    await addTransaction({
      ...
      year: d.getFullYear(),
      month: d.getMonth(),
      person: transferForm.from as Person,
      type: 'transfer',
      category: 'Transfer',
  ...
  return (
    <div className="pb-20 px-4 pt-4 max-w-2xl mx-auto space-y-4 animate-slide-up">
      ...
      {/* Summary: highest spender / earner */}
      <div className="grid grid-cols-2 gap-2">
        <div className="glass-card rounded-xl p-3">
          ...
          <p className="text-sm font-bold text-foreground">
            {PERSON_EMOJI[highestEarner?.person]} {highestEarner?.person}
          </p>
        ...
        <div className="glass-card rounded-xl p-3">
          ...
          <p className="text-sm font-bold text-foreground">
            {PERSON_EMOJI[highestSpender?.person]} {highestSpender?.person}
          </p>
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[380px]">
                  ...
                  <div key={person}>
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-[360px]">
                        <div className={cn('w-20 flex items-center gap-1.5 shrink-0')}>
                        ...
                        <div className="mt-1.5 px-0">
                          <Progress
                            ...
                            className={cn(
                              ...
                              spendPct >= 80 ? '[&>div]:bg-warning' : `[&>div]:${colors.bg.replace('/15', '')}`
                            )}
                          />
                        </div>
                      )}
                    ...
                    {isExpanded && (
                      <div className={cn('rounded-b-xl border-x border-b px-3 py-3 mb-1 space-y-2', colors.border, colors.bg)}>
                        <div className="grid grid-cols-3 gap-2 text-center">
                          <div>
                            <p className="text-[10px] text-muted-foreground">Opening</p>
                            <p className="text-xs font-bold">{fmtK(opening)}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-muted-foreground">Net</p>
                            <p className={cn('text-xs font-bold', income - expense >= 0 ? 'text-success' : 'text-destructive')}>
                              {income - expense >= 0 ? '+' : ''}{fmtK(income - expense)}
                            </p>
                          </div>
                          <div>
                            <p className="text-[10px] text-muted-foreground">Closing</p>
                            <p className={cn('text-xs font-bold', closing >= 0 ? 'text-success' : 'text-destructive')}>
                              {fmtK(closing)}
                            </p>
                          </div>
                        </div>
                        ...
                        {personTxns.length > 0 && (
                          <div className="pt-1">
                            <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide mb-1.5">Recent Transactions</p>
                                ...
                                <div key={t.id} className="flex items-center justify-between text-xs py-1 border-b border-border/20 last:border-0">
                                  <div className="flex items-center gap-1.5">
                                    <span className={cn(
                                      ...
                                      t.type === 'transfer' ? 'bg-primary/20 text-primary' : 'bg-destructive/20 text-destructive'
                                    )}>
            ...
            <div className="flex items-center gap-2 px-2 py-2 border-t border-border/50 mt-1 text-xs font-bold">
              ...
              <span className="flex-1 text-right text-success">{fmt(totalIncome)}</span>
              <span className="flex-1 text-right text-destructive">{fmt(totalExpense)}</span>
              <span className={cn('flex-1 text-right', totalIncome - totalExpense >= 0 ? 'text-success' : 'text-destructive')}>
        ...
        <div className="mt-2 space-y-1.5">
          ...
          <div className="rounded-lg bg-primary/5 border border-primary/15 p-2.5 mb-3">
            <p className="text-[11px] text-muted-foreground">
              <span className="font-semibold text-primary">↔ Balance Shift Only</span> — Internal transfers move money
              between family members but are <em>not counted as income or expense</em>. Only the account balances shift.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select value={transferForm.from} onValueChange={v => setTransferForm(f => ({ ...f, from: v, fromAccount: '' }))}>
            ...
            <div>
              ...
              <Select value={transferForm.to} onValueChange={v => setTransferForm(f => ({ ...f, to: v, toAccount: '' }))}>
          ...
          >
            ...
            Record Transfer (Balance Shift Only)
          </Button>
        </div>
              ...
              {transferHistory.map(t => (
                <div key={t.id} className="flex items-center justify-between text-xs rounded-lg bg-primary/5 px-3 py-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-primary">{fmt(t.amount)}</span>
                  </div>
                </div>
              ))}
